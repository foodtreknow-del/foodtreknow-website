-- FoodTrekNow notifications for meaningful Host edits to posted opportunities.
...
begin
  ...
  change_summary := 'The Host updated ' || array_to_string(changed_labels, ', ') || ' for ' || new.title || '. Open this event to review the latest information.';
  ...
  select
    ...
    a.id,
    'message',
    'opportunity-updated:' || new.id || ':' || a.id || ':' || to_char(coalesce(new.updated_at, now()), 'YYYYMMDDHH24MISSUS'),
    'Opportunity updated',
    change_summary
  from public.opportunity_applications a
