-- Keep Vendor application decisions distinct from Host booking cancellations.
-- Corrects earlier cancellations that were displayed as declined.
...
alter table public.opportunity_applications
  ...
  check (status in ('pending', 'approved', 'declined', 'waitlisted', 'withdrawn', 'cancelled'));
...
-- Repair applications changed to declined by the previous cancellation function.
update public.opportunity_applications a
set status = 'cancelled', updated_at = now()
from public.opportunity_bookings b
where b.application_id = a.id
  and b.status = 'cancelled_by_host'
  and a.status = 'declined'
  and a.host_response like 'Host cancelled approved booking:%';
