(function exposeOpportunityMarketplace() {
  ...
  const state = {
    ...
    host: { profile: null, locations: [], opportunities: [], applications: [], bookings: [], messages: [], reviews: [], notifications: [], stripe: null, tab: 'dashboard', editingOpportunityId: null },
    activeRole: null,
  ...
  function vendorApplicationFor(opportunityId) {
    return state.vendor.applications.find(application => String(application.opportunity_id) === String(opportunityId)) || null;
  }

  function vendorOpportunityAction(item, application, secondary = false) {
    if (application) {
      return `<button class="${secondary ? 'secondary-button' : 'primary-button'}" data-open-vendor-application="${escapeHtml(application.id)}" type="button">View ${escapeHtml(applicationStatusLabel(application.status))} Application</button>`;
    }
    return `<button class="${secondary ? 'secondary-button' : 'primary-button'}" data-marketplace-apply="${escapeHtml(item.id)}" type="button">${item.booking_mode === 'instant' ? 'Book Now' : 'Request Spot'}</button>`;
  }
  ...
  function opportunityCard(item) {
    ...
    const application = vendorApplicationFor(item.id);
    ...
    return `<article class="opportunity-card opportunity-card-clickable" data-opportunity-card="${item.id}" aria-label="View ${escapeHtml(item.title)} opportunity">${photo ? `<img class="opportunity-card-photo" src="${escapeHtml(photo)}" alt="${escapeHtml(item.location?.name)}">` : ''}
      <div class="opportunity-card-top"><div>${availabilityBadge(item)}${application ? `<span class="marketplace-badge application-status">${escapeHtml(applicationStatusLabel(application.status))} application</span>` : ''}${!Number(item.flat_vendor_fee) && !Number(item.sales_percentage) ? '<span class="marketplace-badge no-fee">No Fee</span>' : ''}${Number(item.minimum_sales_guarantee) ? '<span class="marketplace-badge guarantee">Guaranteed Minimum</span>' : ''}</div><button class="opportunity-favorite ${saved ? 'saved' : ''}" data-marketplace-favorite="${item.id}" type="button" aria-label="${saved ? 'Remove saved opportunity' : 'Save opportunity'}">${saved ? '♥' : '♡'}</button></div>
      ...
      <div class="opportunity-card-actions"><button class="secondary-button" data-marketplace-view="${item.id}" type="button">View Opportunity</button>${vendorOpportunityAction(item, application)}</div>
    </article>`;
  }
  ...
  function filterMarkup(todayOnly) {
    ...
    return `<form id="marketplaceFilters" class="marketplace-filters">
      ...
      <label>Event type<select name="eventType"><option value="">All types</option>${['location','lunch','dinner','community','private_event','festival','market','sports','other'].map(value => `<option value="${value}"${selected('eventType', value)}>${value.replaceAll('_', ' ')}</option>`).join('')}</select></label>
  ...
  function unreadVendorMessages(applicationId, section) {
    ...
    if (!field) return 0;
    return state.vendor.messages.filter(message => (!applicationId || message.application_id === applicationId) && message.sender_role === 'host' && !message[field]).length;
  }

  function vendorUnreadButton(applicationId, section, unread) {
    if (!unread) return '';
    const label = `${unread} unread message${unread === 1 ? '' : 's'}`;
    return `<button class="marketplace-unread-badge marketplace-unread-button" data-marketplace-message="${escapeHtml(applicationId)}" data-message-section="${escapeHtml(section)}" type="button" aria-label="Open and read ${label}">${unread}</button>`;
  }
  ...
  function applicationsMarkup() {
    ...
    return `<div class="marketplace-record-list">${state.vendor.applications.map(item => { const unread = unreadVendorMessages(item.id, 'applications'); return `<article data-vendor-application-record="${escapeHtml(item.id)}" class="${unread ? 'marketplace-record-unread' : ''}"><div><span class="status-pill ${item.status}">${escapeHtml(applicationStatusLabel(item.status))}</span><div class="marketplace-record-title"><h3>${escapeHtml(item.opportunities?.title || 'Opportunity')}</h3>${vendorUnreadButton(item.id, 'applications', unread)}</div><p>${escapeHtml(item.opportunities?.host_locations?.name || '')} · ${escapeHtml(dateTime(item.opportunities?.starts_at))}</p>${item.host_response ? `<small>Host response: ${escapeHtml(item.host_response)}</small>` : item.status === 'declined' ? '<small>You may still message the Host with questions about this decision.</small>' : ''}</div><button class="secondary-button" data-marketplace-message="${item.id}" data-message-section="applications" type="button">${unread ? `Read ${unread} New Message${unread === 1 ? '' : 's'}` : 'Reply to Host'}</button></article>`; }).join('')}</div>`;
  }
  ...
  function hostActiveOpportunities() {
    return state.host.opportunities.filter(item => !item.archived_at);
  }

  function hostArchivedOpportunities() {
    return state.host.opportunities.filter(item => item.archived_at);
  }
  ...
  function hostVendorVisibility(item) {
    if (item.archived_at) return { visible: false, label: 'Archived - not visible to vendors' };
    if (item.status !== 'published') return { visible: false, label: `${applicationStatusLabel(item.status)} - not open to vendors` };
    if (new Date(item.ends_at) <= new Date()) return { visible: false, label: 'Event ended - not visible to vendors' };
    return { visible: true, label: 'Visible in the Vendor portal' };
  }
  ...
  function opportunityForm() {
    const editing = Boolean(state.host.editingOpportunityId);
    const termsLocked = editing && state.host.bookings.some(booking => booking.opportunity_id === state.host.editingOpportunityId);
    const feeLock = termsLocked ? ' readonly aria-disabled="true"' : '';
    const locationTypes = [['office', 'Office'], ['business_park', 'Business park'], ['warehouse', 'Warehouse'], ['manufacturing', 'Manufacturing'], ['apartments', 'Apartment community'], ['hoa', 'HOA'], ['church', 'Church'], ['school', 'School'], ['college', 'College'], ['hospital', 'Hospital'], ['dealership', 'Car dealership'], ['construction', 'Construction site'], ['brewery', 'Brewery'], ['sports', 'Sports facility'], ['pool', 'Pool'], ['government', 'Government facility'], ['shopping_center', 'Shopping center'], ['private_property', 'Private property'], ['other', 'Other']];
    const locationFields = `<fieldset class="wide marketplace-location-fields"><legend>Event location</legend><p>Enter the location for this opportunity. It does not have to be selected from a saved-location list.</p><div class="marketplace-form-grid"><label>Location or venue name<input name="locationName" required maxlength="160" placeholder="Community Outreach Center"></label><label>Location type<select name="locationType" required>${locationTypes.map(([value, label]) => `<option value="${value}">${label}</option>`).join('')}</select></label><label class="wide">Street address<input name="locationAddress1" required maxlength="180" autocomplete="street-address"></label><label>Address line 2<input name="locationAddress2" maxlength="180"></label><label>City<input name="locationCity" required maxlength="100" autocomplete="address-level2"></label><label>State<input name="locationState" required maxlength="40" autocomplete="address-level1"></label><label>ZIP/postal code<input name="locationPostalCode" required maxlength="16" autocomplete="postal-code"></label><label>Latitude <small>(optional)</small><input name="locationLatitude" type="number" min="-90" max="90" step="0.000001"></label><label>Longitude <small>(optional)</small><input name="locationLongitude" type="number" min="-180" max="180" step="0.000001"></label></div></fieldset>`;
    return `<form id="hostOpportunityForm" class="marketplace-form marketplace-form-grid"><input name="opportunityId" type="hidden" value="${escapeHtml(state.host.editingOpportunityId || '')}"><label class="wide">Opportunity name<input name="title" required maxlength="180" placeholder="Wednesday Lunch Opening"></label>${locationFields}<label>Opportunity type<select name="opportunityType"><option value="one_time">One time</option><option value="recurring">Recurring</option></select></label><label>Event type<select name="eventType"><option value="location">Location visit</option><option value="lunch">Lunch</option><option value="dinner">Dinner</option><option value="community">Community</option><option value="private_event">Private event</option><option value="festival">Festival</option><option value="market">Market</option><option value="sports">Sports</option><option value="other">Other</option></select></label><label>Booking method<select name="bookingMode"><option value="request">Host approval required</option><option value="instant">Immediate booking</option></select></label><label>Starts<input name="startsAt" type="datetime-local" required></label><label>Ends<input name="endsAt" type="datetime-local" required></label><label>Arrival time<input name="arrivalTime" type="datetime-local"></label><label>Expected customers<input name="expectedCustomers" type="number" min="1" required></label><label>Trucks requested<input name="trucksRequested" type="number" min="1" max="100" required></label><label>Cuisines requested<input name="cuisines" placeholder="BBQ, Mexican, Caribbean"></label><label>Indoor/outdoor<select name="indoorOutdoor"><option value="outdoor">Outdoor</option><option value="indoor">Indoor</option><option value="both">Both</option></select></label>${termsLocked ? '<p class="wide event-fee-guidance"><strong>Financial terms are locked.</strong> A food truck has already been approved for this event. The agreed fees, deposit, and guarantee cannot be changed.</p>' : ''}<p class="wide event-fee-guidance">Flat fees and refundable deposits are collected through Stripe after approval. Connect your Host Stripe account under <strong>Payments</strong> before publishing a paid opportunity. Percentage-of-sales fees are settled separately after the event.</p><label>Flat vendor fee<input name="flatFee" type="number" min="0" step="0.01" value="0"${feeLock}></label><label>Percentage of sales<input name="percentageFee" type="number" min="0" max="100" step="0.01" value="0"${feeLock}></label><label>Minimum sales guarantee<input name="minimumGuarantee" type="number" min="0" step="0.01" value="0"${feeLock}></label><label>Refundable deposit<input name="deposit" type="number" min="0" step="0.01" value="0"${feeLock}></label><label class="wide">Description<textarea name="description" maxlength="2000"></textarea></label><label class="wide">Parking instructions<textarea name="parking" maxlength="1200"></textarea></label><label class="wide">Setup instructions<textarea name="setup" maxlength="1200"></textarea></label><label class="wide">Special requirements<textarea name="requirements" maxlength="1200"></textarea></label><label class="wide">Cancellation policy<textarea name="cancellation" maxlength="1200"></textarea></label><div class="marketplace-filter-checks wide"><label><input name="electricity" type="checkbox"> Electricity available</label><label><input name="water" type="checkbox"> Water available</label></div><div class="recurrence-fields wide"><label>Repeat every<select name="frequency"><option value="weekly">Week</option><option value="daily">Day</option><option value="monthly">Month</option></select></label><label>Until<input name="recurrenceEnds" type="date"></label><div class="marketplace-filter-checks"><label><input name="day" value="1" type="checkbox"> Mon</label><label><input name="day" value="2" type="checkbox"> Tue</label><label><input name="day" value="3" type="checkbox"> Wed</label><label><input name="day" value="4" type="checkbox"> Thu</label><label><input name="day" value="5" type="checkbox"> Fri</label><label><input name="day" value="6" type="checkbox"> Sat</label><label><input name="day" value="0" type="checkbox"> Sun</label></div></div><button class="primary-button" type="submit">${editing ? 'Update Opportunity' : 'Publish Opportunity'}</button><p class="form-message" data-marketplace-form-message aria-live="polite"></p></form>`;
  }

  function hostDashboard() {
    const activeOpportunities = hostActiveOpportunities();
    const activeOpportunityIds = new Set(activeOpportunities.map(item => item.id));
    const open = activeOpportunities.filter(item => ['published', 'filled'].includes(item.status)).length;
    const pending = state.host.applications.filter(item => hostApplicationIsActive(item) && item.status === 'pending').length;
    const approved = state.host.bookings.filter(item => activeOpportunityIds.has(item.opportunity_id) && item.status === 'confirmed').length;
    return `<section class="host-summary-grid"><button class="host-summary-card" data-host-marketplace-tab="opportunities" type="button" aria-label="View ${open} open opportunities"><span>Open Opportunities</span><strong>${open}</strong><small>Click to view</small></button><button class="host-summary-card" data-host-marketplace-tab="applications" type="button" aria-label="View ${pending} pending applications"><span>Pending Applications</span><strong>${pending}</strong><small>Click to view</small></button><button class="host-summary-card" data-host-marketplace-tab="bookings" type="button" aria-label="View ${approved} approved food trucks"><span>Approved Food Trucks</span><strong>${approved}</strong><small>Click to view</small></button><button class="host-summary-card" data-host-marketplace-tab="messages" type="button" aria-label="View ${unreadHostMessages()} unread messages"><span>Unread Messages</span><strong>${unreadHostMessages()}</strong><small>Click to view</small></button></section><div class="host-quick-actions"><button class="primary-button" data-host-marketplace-tab="post" type="button">+ Post Opportunity</button><button class="secondary-button" data-host-marketplace-tab="applications" type="button">Review Applications</button></div><section class="marketplace-record-list">${activeOpportunities.length ? activeOpportunities.slice(0, 5).map(item => `<article><div><span class="status-pill ${item.status}">${escapeHtml(item.status)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(dateTime(item.starts_at))} · ${Number(item.expected_customers).toLocaleString()} expected customers</p><span class="opportunity-interest-count">🚚 ${hostOpportunityInterestCount(item.id)} interested</span></div><button class="secondary-button" data-edit-opportunity="${item.id}" type="button">Edit</button></article>`).join('') : empty('📣', 'No active opportunities', 'Create a new opportunity or open Archive to see completed events.')}</section>`;
  }
  ...
  function hostOpportunitiesMarkup() {
    const opportunities = [...hostActiveOpportunities()].sort((a, b) => new Date(b.starts_at) - new Date(a.starts_at));
    if (!opportunities.length) return empty('📣', 'No opportunities posted', 'Create your first one-time or recurring food truck opening.');
    const vendorVisible = opportunities.filter(item => hostVendorVisibility(item).visible).length;
    return `<div class="marketplace-results-bar"><div><p class="eyebrow">Host Opportunities</p><h2>All Host opportunity records</h2><small>${vendorVisible} visible to vendors · ${opportunities.length - vendorVisible} closed, filled, ended, or Host-only</small></div><button class="primary-button" data-host-marketplace-tab="post" type="button">+ Post Opportunity</button></div><div class="marketplace-record-list">${opportunities.map(item => { const posted = ['published', 'filled'].includes(item.status); const canArchive = new Date(item.ends_at) <= new Date() || ['cancelled', 'completed'].includes(item.status); const visibility = hostVendorVisibility(item); return `<article><div><span class="status-pill ${item.status}">${escapeHtml(item.status)}</span><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(dateTime(item.starts_at))} · ${Number(item.expected_customers).toLocaleString()} expected customers</p><span class="opportunity-interest-count">🚚 ${hostOpportunityInterestCount(item.id)} interested</span><small class="opportunity-visibility ${visibility.visible ? 'visible' : 'hidden-from-vendors'}">${visibility.visible ? '✓' : 'ⓘ'} ${escapeHtml(visibility.label)}</small></div><div class="stacked-actions"><button class="secondary-button" data-edit-opportunity="${item.id}" type="button">Edit</button>${posted ? `<button class="danger-button" data-cancel-host-opportunity="${item.id}" type="button">Cancel Opportunity</button>` : ''}${canArchive ? `<button class="secondary-button" data-archive-host-opportunity="${item.id}" type="button">Archive</button>` : ''}</div></article>`; }).join('')}</div>`;
  }
  ...
  function hostMessagesMarkup() {
    const applications = state.host.applications.filter(hostApplicationIsActive);
    if (!applications.length) return `<div class="opportunity-empty"><span>💬</span><h3>No vendor conversations yet</h3><p>Posting an opportunity does not create a message. A conversation appears here after a food truck requests a spot or books the event.</p><button class="secondary-button" data-host-marketplace-tab="opportunities" type="button">View Posted Opportunities</button></div>`;
    return `<div class="marketplace-record-list">${applications.map(item => { const thread = conversationItems(item, state.host.messages.filter(message => message.application_id === item.id)); const last = thread.at(-1); const unread = unreadHostMessages(item.id); return `<article class="${unread ? 'marketplace-record-unread' : ''}"><div><span class="status-pill ${item.status}">${escapeHtml(item.status)}</span><div class="marketplace-record-title"><h3>${escapeHtml(item.opportunities?.title || 'Opportunity')}</h3>${unread ? `<span class="marketplace-unread-badge" aria-label="${unread} unread message${unread === 1 ? '' : 's'}">${unread}</span>` : ''}</div><p>${escapeHtml(item.trucks?.name || 'Food Truck')}</p><small>${thread.length} message${thread.length === 1 ? '' : 's'} in this event conversation</small><small>${last ? escapeHtml(last.body) : 'No messages yet. Start this event conversation.'}</small></div><button class="primary-button" data-marketplace-message="${item.id}" type="button">${thread.length ? 'Open Conversation' : 'Start Conversation'}</button></article>`; }).join('')}</div>`;
  }
  ...
  function opportunityModal(item) {
    ...
    const estimate = profitability(item, distance);
    const application = vendorApplicationFor(item.id);
    return `<p class="eyebrow">Opportunity Details</p><h2 id="customerModalTitle">${escapeHtml(item.title)}</h2><p>${escapeHtml(item.description || 'The host has not added a description.')}</p>${application ? `<p class="marketplace-application-summary"><strong>${escapeHtml(applicationStatusLabel(application.status))} application:</strong> You already requested this opportunity. Open the application to view its status and continue the event conversation.</p>` : ''}<div class="opportunity-detail-grid"><span><small>Location</small><strong>${escapeHtml(item.location?.name)}</strong><p>${escapeHtml(item.location?.address_line1)}, ${escapeHtml(item.location?.city)}, ${escapeHtml(item.location?.state)} ${escapeHtml(item.location?.postal_code)}</p></span><span><small>Date and time</small><strong>${escapeHtml(dateTime(item.starts_at))}</strong><p>Ends ${escapeHtml(dateTime(item.ends_at))}</p></span><span><small>Expected customers</small><strong>${Number(item.expected_customers).toLocaleString()}</strong></span><span><small>Competition</small><strong>${item.trucks_booked} booked · ${capacityLeft(item)} open</strong></span><span><small>Vendor fee</small><strong>${escapeHtml(feeLabel(item))}</strong></span><span><small>Minimum guarantee</small><strong>${money(item.minimum_sales_guarantee)}</strong></span></div><section class="profitability-detail"><div><span class="profitability-score">${estimate.score}</span><div><p class="eyebrow">Profitability Score</p><h3>${estimate.label}</h3></div></div><div class="profitability-reasons"><section><strong>Positive</strong>${estimate.positive.map(reason => `<p>+ ${escapeHtml(reason)}</p>`).join('') || '<p>No strong positive signals yet.</p>'}</section><section><strong>Watch</strong>${estimate.negative.map(reason => `<p>− ${escapeHtml(reason)}</p>`).join('') || '<p>No major risks identified.</p>'}</section></div><div class="estimate-table"><span>Projected sales <strong>${money(estimate.projectedSales)}</strong></span><span>Vendor fees <strong>−${money(estimate.vendorFee)}</strong></span><span>Estimated food cost <strong>−${money(estimate.foodCost)}</strong></span><span>Estimated labor <strong>−${money(estimate.laborCost)}</strong></span><span>Estimated travel <strong>−${money(estimate.travelCost)}</strong></span><span class="total">Estimated profit <strong>${money(estimate.estimatedProfit)}</strong></span></div><p class="estimate-disclaimer">This is an estimate based on host-provided attendance and planning assumptions. FoodTrekNow does not guarantee sales or income.</p></section><section><h3>Setup and requirements</h3><p><strong>Parking:</strong> ${escapeHtml(item.parking_instructions || 'Not specified')}</p><p><strong>Setup:</strong> ${escapeHtml(item.setup_instructions || 'Not specified')}</p><p><strong>Special requirements:</strong> ${escapeHtml(item.special_requirements || 'None listed')}</p><p><strong>Cancellation:</strong> ${escapeHtml(item.cancellation_policy || 'Ask the host before booking.')}</p></section><div class="opportunity-card-actions">${vendorOpportunityAction(item, application)}</div>`;
  }

  function truckProfileModal(profile) {
    const truck = profile?.truck || {};
    const rating = profile?.rating || {};
    const items = asArray(profile?.menu_items);
    const categories = asArray(profile?.categories);
    const reviews = asArray(profile?.reviews);
    const hours = asArray(profile?.hours);
    const logo = safeImageUrl(truck.logo_url);
    const ratingCount = Number(rating.count || 0);
    const ratingLabel = ratingCount ? `${Number(rating.average || 0).toFixed(1)} ★ · ${ratingCount} Host review${ratingCount === 1 ? '' : 's'}` : 'New truck · No Host reviews yet';
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const timeLabel = value => { if (!value) return ''; const [hourValue, minute] = String(value).split(':'); const hour = Number(hourValue); return `${hour % 12 || 12}:${minute} ${hour >= 12 ? 'PM' : 'AM'}`; };
    const hoursMarkup = hours.length ? hours.map(row => `<span><small>${dayNames[Number(row.day_of_week)] || 'Day'}</small><strong>${row.is_closed ? 'Closed' : `${timeLabel(row.opens_at)}–${timeLabel(row.closes_at)}`}</strong></span>`).join('') : '<p class="truck-profile-empty">Hours have not been published.</p>';
    const menuSections = categories.map(category => ({ ...category, items: items.filter(item => item.category_id === category.id) }));
    const uncategorized = items.filter(item => !categories.some(category => category.id === item.category_id));
    if (uncategorized.length) menuSections.push({ id: 'other', name: 'Other', description: '', items: uncategorized });
    const menuMarkup = menuSections.filter(section => section.items.length).map(section => `<section class="host-truck-menu-section"><div><h3>${escapeHtml(section.name)}</h3>${section.description ? `<p>${escapeHtml(section.description)}</p>` : ''}</div><div class="host-truck-menu-grid">${section.items.map(item => { const photo = safeImageUrl(item.photo_url); return `<article class="host-truck-menu-item ${item.is_sold_out ? 'sold-out' : ''}">${photo ? `<img src="${escapeHtml(photo)}" alt="${escapeHtml(item.name)}">` : '<div class="host-truck-menu-placeholder" aria-hidden="true">🍽️</div>'}<div><span>${item.is_featured ? 'Featured' : escapeHtml(section.name)}</span><h4>${escapeHtml(item.name)}</h4><p>${escapeHtml(item.description || 'No description provided.')}</p><div><strong>${money(item.price)}</strong>${item.is_sold_out ? '<b>SOLD OUT</b>' : ''}</div></div></article>`; }).join('')}</div></section>`).join('');
    const reviewsMarkup = reviews.length ? reviews.map(review => `<article><strong>${'★'.repeat(Number(review.rating || 0))}${'☆'.repeat(Math.max(0, 5 - Number(review.rating || 0)))}</strong><p>${escapeHtml(review.comment || 'Rating submitted without a written comment.')}</p><small>${new Date(review.created_at).toLocaleDateString([], { year: 'numeric', month: 'short', day: 'numeric' })}</small></article>`).join('') : '<p class="truck-profile-empty">No Host reviews have been submitted for this truck yet.</p>';
    return `<div class="host-truck-profile"><header>${logo ? `<img src="${escapeHtml(logo)}" alt="${escapeHtml(truck.name || 'Food truck')} logo">` : '<div class="host-truck-profile-logo" aria-hidden="true">🚚</div>'}<div><p class="eyebrow">Customer-facing food truck profile</p><h2 id="customerModalTitle">${escapeHtml(truck.name || 'Food Truck')}</h2><p>${escapeHtml(truck.cuisine || 'Cuisine not listed')} · ${escapeHtml(ratingLabel)}</p></div></header><p class="host-truck-description">${escapeHtml(truck.description || 'This food truck has not added a description yet.')}</p><div class="host-truck-facts"><span><small>Ordering</small><strong>${truck.accepting_orders ? 'Accepting orders' : 'Currently unavailable'}</strong></span><span><small>Typical prep</small><strong>${Number(truck.estimated_prep_minutes || 0)} minutes</strong></span><span><small>Minimum order</small><strong>${money(truck.minimum_order)}</strong></span><span><small>Location</small><strong>${escapeHtml(truck.location_name || 'Mobile food truck')}</strong></span></div><section class="host-truck-rating"><div><p class="eyebrow">Host ratings</p><h3>${escapeHtml(ratingLabel)}</h3></div>${ratingCount ? `<div class="host-truck-rating-grid"><span>Communication <strong>${Number(rating.communication || 0).toFixed(1)}</strong></span><span>Accuracy <strong>${Number(rating.accuracy || 0).toFixed(1)}</strong></span><span>Setup <strong>${Number(rating.setup || 0).toFixed(1)}</strong></span></div>` : ''}</section><section><h3>Published hours</h3><div class="host-truck-hours">${hoursMarkup}</div></section><section><div class="host-truck-menu-heading"><div><p class="eyebrow">Public customer menu</p><h3>Menu &amp; prices</h3></div><small>${items.length} item${items.length === 1 ? '' : 's'}</small></div>${menuMarkup || '<p class="truck-profile-empty">This truck has not published menu items yet.</p>'}</section><section class="host-truck-reviews"><h3>Recent Host reviews</h3>${reviewsMarkup}</section><p class="estimate-disclaimer">Menu prices and availability are supplied by the food truck and may change. Private owner, banking, and account information is never shown.</p></div>`;
  }
  ...
  function cancelHostOpportunityModal(opportunity) {
    ...
    const payments = bookings.map(item => relatedOne(item.event_fee_payments)).filter(Boolean);
    const paid = payments.filter(item => item.status === 'paid');
    const checkoutOpen = payments.some(item => item.status === 'checkout_open');
    const refundTotal = paid.reduce((sum, item) => sum + Number(item.amount_due_cents || 0), 0) / 100;
    const paymentNote = checkoutOpen
      ? '<p class="event-fee-guidance"><strong>A food truck currently has Stripe Checkout open.</strong> For payment safety, try again after that short payment session expires.</p>'
      : paid.length
        ? `<p class="event-fee-guidance"><strong>${money(refundTotal)} across ${paid.length} paid event payment${paid.length === 1 ? '' : 's'} will be refunded automatically through Stripe</strong> before the event is cancelled.</p>`
        : '<p class="event-fee-guidance">No completed event payments need to be refunded.</p>';
    return `<form id="cancelHostOpportunityForm" data-opportunity-id="${escapeHtml(opportunity.id)}" class="marketplace-form"><p class="eyebrow">Posted Opportunity</p><h2 id="customerModalTitle">Cancel ${escapeHtml(opportunity.title)}?</h2><p>This cancels the entire event, not just one food truck. <strong>${applications.length} connected food truck${applications.length === 1 ? '' : 's'}</strong> will receive your reason as an unread message and cancellation notification. Approved bookings will be cancelled and the event will move to Archive.</p>${paymentNote}<label>Why is this event being cancelled?<textarea name="reason" required minlength="3" maxlength="600" rows="5" placeholder="Example: The venue is no longer available due to an unexpected closure."></textarea><small>This message will be sent to every connected food truck.</small></label><div class="stacked-actions"><button class="secondary-button" data-close-customer-modal type="button">Keep Opportunity</button><button class="danger-button" type="submit" ${checkoutOpen ? 'disabled title="Wait for the active Stripe Checkout session to expire"' : ''}>Cancel Entire Event${paid.length ? ' & Refund Payments' : ''}</button></div><p class="form-message" data-marketplace-form-message></p></form>`;
  }

  async function act(button, task, success) {
    const formMessage = button.closest('form')?.querySelector('[data-marketplace-form-message]');
    const originalLabel = button.textContent;
    button.disabled = true;
    button.textContent = 'Saving…';
    if (formMessage) { formMessage.textContent = 'Saving your changes…'; formMessage.classList.remove('success'); }
    try {
      await task();
      if (formMessage?.isConnected) { formMessage.textContent = success; formMessage.classList.add('success'); }
      toast(success);
    }
    catch (error) {
      const message = error?.message || 'The request could not be completed.';
      if (formMessage) { formMessage.textContent = message; formMessage.classList.remove('success'); }
      toast(message, true);
    }
  ...
  document.addEventListener('click', async event => {
    ...
    if (hostTab) { state.host.tab = hostTab.dataset.hostMarketplaceTab; if (state.host.tab === 'post') state.host.editingOpportunityId = null; renderHostRoot(); return; }
    ...
    if (openVendorApplication) {
      ...
      document.getElementById('marketplaceModal')?.classList.add('hidden');
      renderVendorRoot();
      document.querySelector(`[data-vendor-application-record="${openVendorApplication.dataset.openVendorApplication}"]`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    ...
    const cancelHostOpportunity = event.target.closest('[data-cancel-host-opportunity]');
    if (cancelHostOpportunity) {
      const opportunity = state.host.opportunities.find(item => item.id === cancelHostOpportunity.dataset.cancelHostOpportunity);
      if (opportunity) openMarketplaceModal(cancelHostOpportunityModal(opportunity));
      return;
    }
    const archiveOpportunity = event.target.closest('[data-archive-host-opportunity]');
    if (archiveOpportunity) {
      if (!window.confirm('Archive this completed or cancelled event? Its applications and messages will remain available in Archive.')) return;
      await act(archiveOpportunity, async () => {
        await rpc('archive_host_opportunity', { p_opportunity_id: archiveOpportunity.dataset.archiveHostOpportunity });
        await loadHostData();
        state.host.tab = 'archive';
        renderHostRoot();
      }, 'Event moved to Archive.');
      return;
    }
    const restoreOpportunity = event.target.closest('[data-restore-host-opportunity]');
    if (restoreOpportunity) {
      await act(restoreOpportunity, async () => {
        await rpc('restore_host_opportunity_archive', { p_opportunity_id: restoreOpportunity.dataset.restoreHostOpportunity });
        await loadHostData();
        state.host.tab = 'opportunities';
        renderHostRoot();
    ...
    if (message) {
      ...
      try {
        if (state.activeRole === 'vendor') {
          const section = message.dataset.messageSection || (['applications', 'messages', 'bookings'].includes(state.vendor.tab) ? state.vendor.tab : 'messages');
          await rpc('mark_vendor_opportunity_section_read', { p_application_id: state.selectedApplication, p_section: section });
          const field = { applications: 'vendor_applications_read_at', messages: 'vendor_messages_read_at', bookings: 'vendor_bookings_read_at' }[section];
          state.vendor.messages.forEach(item => { if (item.application_id === state.selectedApplication && item.sender_role === 'host') item[field] = item[field] || new Date().toISOString(); });
          renderVendorRoot();
        } else {
    ...
    if (editOpportunity) {
      ...
      const location = state.host.locations.find(entry => entry.id === item.location_id) || {};
      state.host.editingOpportunityId = item.id; state.host.tab = 'post'; renderHostRoot(); const form = document.getElementById('hostOpportunityForm'); if (!form) return;
      const localInput = value => value ? new Date(value).toISOString().slice(0, 16) : '';
      const values = { opportunityId: item.id, locationName: location.name, locationType: location.location_type, locationAddress1: location.address_line1, locationAddress2: location.address_line2, locationCity: location.city, locationState: location.state, locationPostalCode: location.postal_code, locationLatitude: location.latitude, locationLongitude: location.longitude, title: item.title, description: item.description, opportunityType: item.opportunity_type, eventType: item.event_type, bookingMode: item.booking_mode, startsAt: localInput(item.starts_at), endsAt: localInput(item.ends_at), arrivalTime: localInput(item.arrival_time), expectedCustomers: item.expected_customers, trucksRequested: item.trucks_requested, cuisines: asArray(item.cuisine_preferences).join(', '), indoorOutdoor: item.indoor_outdoor, flatFee: item.flat_vendor_fee, percentageFee: item.sales_percentage, minimumGuarantee: item.minimum_sales_guarantee, deposit: item.refundable_deposit, parking: item.parking_instructions, setup: item.setup_instructions, requirements: item.special_requirements, cancellation: item.cancellation_policy };
  ...
  document.addEventListener('submit', async event => {
    ...
    if (event.target.id === 'hostOpportunityForm') {
      ...
      await act(button, async () => {
        const result = await rpc('publish_opportunity_with_location', {
          p_opportunity_id: opportunityId,
          p_location_name: data.get('locationName'),
          p_location_type: data.get('locationType'),
          p_location_address_line1: data.get('locationAddress1'),
          p_location_address_line2: data.get('locationAddress2'),
          p_location_city: data.get('locationCity'),
          p_location_state: data.get('locationState'),
          p_location_postal_code: data.get('locationPostalCode'),
          p_location_latitude: data.get('locationLatitude') || null,
          ...
          p_special_requirements: data.get('requirements'), p_cancellation_policy: data.get('cancellation'),
          p_recurrence: data.get('opportunityType') === 'recurring' ? { frequency: data.get('frequency'), interval_count: 1, days_of_week: days, recurrence_ends_on: data.get('recurrenceEnds') || null } : null
        });
    ...
    if (event.target.id === 'cancelHostOpportunityForm') {
      event.preventDefault(); const button = event.target.querySelector('button[type="submit"]'); const data = new FormData(event.target);
      const opportunity = state.host.opportunities.find(item => item.id === event.target.dataset.opportunityId);
      if (!opportunity) { toast('The posted opportunity could not be found.', true); return; }
      await act(button, async () => {
        const bookings = state.host.bookings.filter(item => item.opportunity_id === opportunity.id && item.status === 'confirmed');
        const paidPayments = bookings.map(item => relatedOne(item.event_fee_payments)).filter(payment => payment?.status === 'paid');
        for (const payment of paidPayments) {
          await edge('stripe-event-fee-refund', { paymentId: payment.id, confirmed: true });
        }
        await rpc('cancel_host_opportunity', { p_opportunity_id: opportunity.id, p_reason: data.get('reason') });
        await loadHostData();
        state.host.editingOpportunityId = null;
        state.host.tab = 'archive';
        document.getElementById('customerAccountModal')?.classList.add('hidden');
        document.getElementById('marketplaceModal')?.classList.add('hidden');
        renderHostRoot();
      }, 'Event cancelled. Connected food trucks were notified and the event was archived.'); return;
    }
